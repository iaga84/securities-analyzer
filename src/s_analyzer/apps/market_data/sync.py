...
def refresh_moving_average_records(security_pk):
    security = Security.objects.get(pk=security_pk)
    periods = SecurityDailyMovingAveragePeriod.objects.filter(security__pk=security_pk)
    today = datetime.today()

    for period in periods:
        logger.info('calculating moving average for {} [{}]'.format(security, period.days))
        historical_values = SecurityDailyData.objects.filter(security__pk=security_pk).order_by('-date')[:period.days]
        if historical_values.count() < period.days:
            logger.info('not enough items for {}'.format(security))
        else:
            assert historical_values.count() == period.days
            avg = historical_values.aggregate(Avg('adjusted_closing_price'))

            SecurityDailyMovingAverage.objects.update_or_create(
                period=period, date=today,
                defaults={
                    'average': avg['adjusted_closing_price__avg'],
                },
            )
